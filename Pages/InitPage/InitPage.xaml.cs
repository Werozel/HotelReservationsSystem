    ...
    public sealed partial class InitPage : Page
    {
        public InitPage()
        {
            ...
            (this.StartButton).Click += (s, e) =>
            {
                ...
                success &= int.TryParse(this.DiscountTB.Text, out int discount);
                success &= 0 <= discount && discount <= 100;

                if (success)
                {
                    this.Frame.Navigate(typeof(ExperimentPage.ExperimentPage), new ExperimentParameters(roomsMap, daysCount, hoursPerStep, maxDaysToBook, discount));
                } else
                {
